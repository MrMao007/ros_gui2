...
namespace scan_map_icp
{
  ...
  private:
    ...
    ros::Publisher control_cmd;
    ros::Publisher laser_point_pub;
    ros::Publisher test;
    ros::Publisher target_pub;
    ros::Publisher source_pub;
    ros::Publisher dlf_icp_result;
    //　发布变换后的点云数据
    ...
    sensor_msgs::PointCloud2 ref_cloud2_;
    ...
    tf::Transform t_;
  private:
    // transform convert matrix form ----> 坐标变换转化为矩阵形式
   ...
   // void publishRelocationPose();
    ...
    // 存入参考点云信息
    void ReflaserScanSubCallback(const sensor_msgs::LaserScanConstPtr &msg);
