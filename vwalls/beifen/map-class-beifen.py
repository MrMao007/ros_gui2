...
class Robot:
    ...
    def callback(self,config,level):
        rospy.loginfo("""参数更新:最大墙宽：{max_width_times}, 最小墙宽：{min_width}, 墙长度：{length}, 转弯半径：{r}""".format(**config))
        self.maxwidth_times = config['max_width_times']
    ...
    def dynamic(self):
        if self.in_flag == 0:
            ...
            k,walls = self.door_boundary_point(self.maxwidth_times,self.length,self.r)
