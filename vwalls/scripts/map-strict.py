...
class Robot:
    def __init__(self):
        ...
        self.origin = [-63.053811, -27.172959]
        self.reslution = 0.05
        self.maxwidth = 1
        self.minwidth = 0.5
        self.length = 0.8
    ...
    def callback(self,config,level):
        rospy.loginfo("""参数更新:最大墙宽：{max_width}/m, 最小墙宽：{min_width}/m, 墙长度：{length}/m, 转弯半径：{r}/m""".format(**config))
    ...
    def dynamic(self):
        ...
        strictk,w = self.door_boundary_point(1.5,0.2,0)
        self.in_flag = self.pass_process(self.pose,strictk)
        return walls
