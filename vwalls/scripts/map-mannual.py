...
class Robot:
    def __init__(self):
        ...
        self.map = cv2.imread(rospy.get_param('~door_map'))
        self.origin = rospy.get_param('~origin')
        self.reslution = rospy.get_param('~resolution')
        self.maxwidth = rospy.get_param('~maxwidth')
        self.minwidth = rospy.get_param('~minwidth')
        self.length = rospy.get_param('~length')
        self.r = rospy.get_param('~r')
        self.bias_x = rospy.get_param('~bias_x')
        self.bias_y = rospy.get_param('~bias_y')
        self.in_flag = 0
        self.door_flag = -1
        self.corner_flag = -1
    ...
    def callback(self,config,level): 
        rospy.loginfo("""最大墙宽：{max_width}m, 最小墙宽：{min_width}m, 墙长：{length}m, 转弯半径：{r}m, x方向偏移：{bias_x}m， y方向偏移：{bias_y}m""".format(**config))
        self.maxwidth = config['max_width']
        self.minwidth = config['min_width']
        self.length = config['length']
        self.r = config['r']
        self.bias_x = config['bias_x']
        self.bias_y = config['bias_y']
        return config
    ...
    def door_msg(self):
        ...
        for i in range(len(contours)):
            ...
            if (area < 1000):
                ...
                doorsp = []
                raw_center = self.point_converter(rect[0])
                ...
                if (height > width):
                    correct_center= raw_center + height_v/np.linalg.norm(height_v)*self.bias_x + width_v/np.linalg.norm(width_v)*self.bias_y
                    doorsp.append(correct_center)
                    doorsp.append(height*self.reslution)
                    if (box[2][1] > box[1][1]):
                        width_v = box[1] - box[2]
                    cos = width_v.dot(p0)/width
                else:
                    correct_center= raw_center + width_v/np.linalg.norm(width_v)*self.bias_x + height_v/np.linalg.norm(height_v)*self.bias_y
                    doorsp.append(correct_center)
    ...
    def door_boundary_point(self,l,w,r):
        ...
        if self.door_to_pass:
            ...
            circle = Point()
            Form_circle = Form()
            if(self.door_to_pass[1] == self.door_flag):
                if(self.corner_flag > 0):
                    if(tgid != self.corner_flag):
                        tgid = self.corner_flag
            else:
                print("door changes to",self.door_to_pass[1],"corner is",tgid)

            if(self.in_flag == 1):
                r = 0
            if tgid > 1:
                circle.x = door_bpoint[tgid][0] + 0.45*circle_dirx
                circle.y = door_bpoint[tgid][1] + 0.45*circle_diry
                circle.z = r
            else:
                circle.x = door_bpoint[tgid][0] - 0.45*circle_dirx
                circle.y = door_bpoint[tgid][1] - 0.45*circle_diry
            ...
            self.door_flag = self.door_to_pass[1]
            self.corner_flag = tgid
